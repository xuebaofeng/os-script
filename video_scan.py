...
def score_video(info, file_size):
    score = 0
    # 编码不是 H.264 / H.265
    if info.get("codec_name") not in ["h264", "hevc"]:
        score += 1
    # 视频码率过高 (>10Mbps)
    try:
...
if videos_to_transcode:
    ...
    for v in videos_to_transcode:
        output_file = os.path.splitext(v['path'])[0] + "_transcoded.mp4"
        print(f'ffmpeg -i "{v["path"]}" -c:v libx265 -preset fast -crf 23 -c:a aac "{output_file}"')
else:
    print("没有找到明显需要转码的视频。")